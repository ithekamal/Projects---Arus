...
function sorting(teamwithRank: any) {
    ...
    rank = rank.sort(st)
    for (let eachRank of rank) {
        for (let eachTeamRank in teamwithRank) {
            if (eachRank == teamwithRank[eachTeamRank]['rank']) {
                sorted.push(teamwithRank[eachTeamRank])
            }
        }
    }
    return sorted
}
let sortedList: any = sorting(playerInfo)

//function to assign points
let player1Point = 100;
sortedList[0]["points"] = 100;
let percentagetoReduce = 256 / sortedList.length;
function rankToPercentage(playerPoint: any, percentage: any) {
    let winningPercentage = playerPoint - playerPoint * (percentage / 100);
    let player1Point = winningPercentage;
    return winningPercentage;
}
for (let i = 1; i < sortedList.length; i++) {
    sortedList[i]["points"] = rankToPercentage(player1Point, percentagetoReduce);
}
let sortedPlayers = sortedList
...
function probabilityOfWin(player1: any, player2: any, main: any) {
    ...
    let totalPointsOfPlayers = player1.points + player2.points;
    let player1Probability = player1.points * 100 / totalPointsOfPlayers;
...
function allResults(matchOutput: any) {
    ...
    for (let eachMatch = 0; eachMatch < matchOutput.length; eachMatch++) {
        matchesByround.push(matchOutput[eachMatch].round)
    }
